...
import org.westernacher.solutions.domain.models.service.SavedJobServiceModel;
...
import org.westernacher.solutions.repository.SavedJobRepository;
...
public class UserServiceImpl implements UserService
{
    ...
    @Autowired
    public UserServiceImpl(UserRepository userRepository, RoleRepository roleRepository, ModelMapper modelMapper, BCryptPasswordEncoder bCryptPasswordEncoder, UserRoleService userRoleService, UserRoleFactory userRoleFactory, SavedJobRepository savedJobRepository) {
...
//                user.setAuthorities(this.getAuthorities("ROLE_USER"));
    ...
    @Override
    public List<SavedJob> getUserCart(String username)
    {
    ...
    public ResponseEntity saveOrder(SavedOrderBindingModel savedOrderBindingModel)
    {
        try
        {
            ...
            SavedJobServiceModel savedApplicationServiceModel = new SavedJobServiceModel();
            modelMapper.map(savedOrderBindingModel, savedApplicationServiceModel);
            savedApplicationServiceModel.setUser(user.get());
            this.savedJobRepository.save(modelMapper.map(savedApplicationServiceModel, SavedJob.class));
            return ResponseEntity.ok().body(true);
        }
        catch(Exception e)
        {
